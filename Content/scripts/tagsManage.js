var myModule = angular.module('TagsManage', ['ui.bootstrap']);
myModule
    .controller("tagsManageCtl", function ($scope, $http, $modal) {
        $scope.totalItems = 0;
        $scope.currentPage = 1;
        $scope.pageSize = 5;
        $scope.PresModel = {};
        ...
        var getRouces = function (page) {
            $http.post("/Admin/GetAllTags", { title: $scope.searchVal, isAwake: $scope.resourceType, pageIndex: page, pageCount: 5 }).success(function (data) {
        ...
        $scope.PResModify = function (PResouce, type) {
            ...
            if (type == "2") {
                $http.post("/Admin/AwakeTag", { TagId: PResouce.Id }).success(function (data) {
                    alert("修改成功！");
                    getRouces(1);
                });
            }
        ...
        $scope.BatchPResDel = function () {
            ...
            if ($scope.chooseList.length > 0 && confirm("确定要删除？")) {
                $http.post("/Admin/DeleteTag", { ids: seri }).success(function (data) {
                    if (!data.Success) {
                        alert("删除失败！");
                    } else {
                        alert("删除成功！");
    ...
    .controller("ModalModifyCtrl", function ($scope, $http, $modalInstance, PResouce) {
        ...
        $scope.PresModify = function () {
            $http.post("/Admin/PostTag", { model: $scope.PRes }).success(function (data) {
                if (!data.Success) {
                    alert("操作失败");
                } else {
                    alert(data.Message);
    ...
    //创建标签
    .controller("CreateTag", function ($scope, $http) {
        $scope.PResouce = {
            Id: null,
            TagName: '',
        };
        $scope.TagSubmit = function () {
            $http.post("/Admin/PostTag", { model: $scope.PResouce }).success(function (data) {
                if (!data.Success) {
                    alert("创建失败！");
                } else {
                    alert("创建成功！");
                    $scope.PResouce.TagName = '';
                    window.location.href = "/Admin/TagsManage"
                }
            });
        }
    })